    ...
    for (let i = 1; i <= poolSize; i++) {
        ...
        try {
            ...
            if (error.statusCode === 404) {
                ...
                const workerEnvFile = process.env.WORKER_ENV_FILE;
                const binds: string[] = [`${volumeName}:/home/vscode/data`];
                if (workerEnvFile) {
                    binds.push(`${workerEnvFile}:/run/secrets/worker.env:ro`);
                }
                const env: string[] = [
                    `MONITOR_COUNT=${monitorCount}`,
                    ...(workerEnvFile ? ['ENV_FILE=/run/secrets/worker.env'] : []),
                ];
